...
const routes = [{url: "/ingredients", label: "Skadniki"}, {url: "/recipes", label: "Przepisy"}];
        ...
        <div className="my-auto flex h-64 w-full flex-col items-center justify-evenly">
          {routes.map((link) => (
              <div key={link.url} className="mx-32 w-80 max-w-sm">
                <Link
                    href={link.url}
                    className="flex grow-1 items-center justify-center rounded-md border border-transparent bg-primary-bg px-4 py-3 text-xl font-medium text-white shadow-xl active:bg-yellow-800 sm:px-8"
                >
                  {link.label}
                </Link>
              </div>
